...
async function handleMenuDate(ctx, dateStr) {
  ...
  ctx.session.menuCreation.displayDate = validation.displayDate;
  ctx.session.menuCreation.step = "items";
  ctx.session.menuCreation.items = [];

  // Клавиатура с действиями (только после завершения)
  const keyboard = Markup.inlineKeyboard([
    [Markup.button.callback("✅ Завершить и сохранить", "kitchen_finish")],
    [Markup.button.callback("❌ Отменить", "kitchen_cancel")],
  ]);
  ...
  await ctx.reply(
    `✅ Дата установлена: **${validation.displayDate}**\n\n` +
      ...
      "Цену можно указывать с валютой или без, бот поймет\n" +
      "⚠️ **У каждого блюда должна быть указана цена!**\n\n" +
      "🔽 **Просто отправляйте блюда одно за другим. После каждого отправления бот будет показывать текущий список.**\n\n" +
...
// Обработка ввода блюда — показывает текущий список
async function handleMenuItem(ctx, line) {
  if (!ctx.state.user) return;
  ...
  // Парсим строку
  const parsed = parseMenuItem(line);
  ...
  // Формируем сообщение с текущим списком блюд
  let responseText = `✅ **Блюдо добавлено!**\n\n`;
  responseText += `📋 **Текущее меню (всего ${menuCreation.items.length} блюд):**\n`;

  menuCreation.items.forEach((item, index) => {
    responseText += `${index + 1}. **${item.name}** — ${item.price}₽\n`;
  });

  responseText += `\n🔽 **Отправьте следующее блюдо или нажмите кнопку ниже для завершения** 🔽`;
