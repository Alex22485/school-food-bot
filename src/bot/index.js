...
bot.use(authMiddleware); // Аутентификация - должна быть перед обработчиками!
...
// Команда /kitchen для быстрого доступа к панели кухни
bot.command(
  "kitchen",
  allowKitchen,
...
// Обработчик для кнопки "Меню" в главном меню родителя
...
// Обработчик для кнопки "Меню" в главном меню пользователя
...
// Временный обработчик для заказа
bot.action(/^order_from_menu_(\d+)$/, allowUser, async (ctx) => {
  await ctx.answerCbQuery("⏳ Функция заказа в разработке");
  await ctx.reply("📝 Функция заказа будет доступна в следующем обновлении!");
});
