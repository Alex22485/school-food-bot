...
function allowRoles(allowedRoles = []) {
    return async function roleMiddleware(ctx, next) {
        try {
            ...
            // Добавляем информацию о разрешенных ролях в состояние
...
const allowUser = allowRoles(['user', 'admin', 'class_teacher', 'kitchen']);
const allowParent = allowRoles(['admin', 'parent']); // parent - это роль для родителей
