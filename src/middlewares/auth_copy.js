...
async function authMiddleware(ctx, next) {
  ...
  try {
    // Только проверяем существование пользователя, НЕ создаем
    const user = prepare(`
            SELECT id, telegram_id, role, city_id, school_id,
                   class_name, child_name, shift, is_active
            FROM users 
            WHERE telegram_id = ?
        `).get(telegramId);
    ...
    if (user) {
      ctx.state.user = user;
      logger.debug("Auth: User found", { userId: user.id });
    } else {
      ctx.state.user = null;
      logger.info("Auth: New user (not in DB)", { telegramId });
    }
  } catch (error) {
